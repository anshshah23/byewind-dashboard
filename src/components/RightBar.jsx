...
const IconWrapper = ({ children, bgColor }) => (
    <div className={`p-1 rounded-full mr-2 ${bgColor}`}>
        {children}
    </div>
);
...
const NotificationItem = ({ notif, isDarkMode }) => {
    const icons = {
        bug: <PiBugBeetle className={`${isDarkMode ? 'text-red-400' : 'text-red-600'}`} style={{ fontSize: '18px' }} />,
        user: <PiUser className={`${isDarkMode ? 'text-blue-400' : 'text-blue-600'}`} style={{ fontSize: '18px' }} />,
        subscribe: <PiBroadcast className={`${isDarkMode ? 'text-green-400' : 'text-green-600'}`} style={{ fontSize: '18px' }} />,
    };
...
export default function Sidebar() {
    ...
    const contactsData = [
        { user: 'Natali Craig' }, { user: 'Drew Cano' }, { user: 'Orlando Diggs' },
        { user: 'Andi Lane' }, { user: 'Kate Morrison' }, { user: 'Koray Occumos' },
    ];
    ...
    return (
        <motion.div
            ...
            className={`max-h-screen font-sans overflow-scroll ${isDarkMode ? 'bg-zinc-900 text-zinc-300' : 'bg-white text-zinc-700'}`}
        >
            <div className="p-4">
                ...
                <Section title="Contacts" isDarkMode={isDarkMode}>
                    {contactsData.map((contact, index) => (
                        <ContactItem key={index} contact={contact} index={index} isDarkMode={isDarkMode} />
                    ))}
                </Section>
            </div>
        </motion.div>

		...
		<>
			...
			>
				...
				<motion.div
					...
					className={`flex flex-col md:flex-row w-full ${isDarkMode
						? "bg-zinc-900 text-white fade-in"
						: "bg-white text-zinc-900 fade-out"
						}`}
				>
					<div className="flex flex-col md:flex-row gap-x-4">
							...
							{boxes.map((data, index) => {
								...
								return (
									<Link
										to={data.link}
										key={index}
										className={`${alternateColor} py-4 md:pt-8 px-6 h-[48%] rounded-3xl w-full md:w-[47%] duration-1000 group`}
									>
										<div className="text-md font-bold">{data.name}</div>
										<div className="mt-4 flex justify-between items-center">
											<span className="text-2xl font-bold">{data.number}</span>
											<span
												className={`text-sm flex items-center ${data.hike.startsWith("-")
													? "text-red-500"
													: "text-green-500"
						...
						<div
							className={`w-full md:w-1/2 flex items-center cursor-pointer rounded-3xl justify-center ${isDarkMode
				...
				>
					<div
						className={`w-full md:w-[70%] rounded-3xl pt-8 pb-12 px-8 ${isDarkMode
							? "bg-zinc-800 hover:bg-zinc-700 text-white fade-in"
							: "bg-zinc-100 hover:bg-zinc-200 text-zinc-900 fade-out"
							}`}
						style={{ height: "auto" }} // Changed to auto for responsive height
					>
						<div className="flex flex-col md:flex-row items-center md:space-x-4 mb-4">
							<h3
								className={`font-bold ${isDarkMode ? "text-white fade-in" : "text-black fade-out"
									}`}
							>
								Revenue
							</h3>
							...
							{revenueData.map((data, index) => (
								...
								>
									<span
										className={`w-2 h-2 rounded-full hidden md:block ${isDarkMode ? "bg-white fade-in" : "bg-black fade-out"
											}`}
									></span>
									<span
							...
							<ResponsiveContainer width="100%" height="100%">
								<LineChart data={data}>
									...
									<Line
										...
										activeDot={{ r: 8 }}
									/>
									<Line type="monotone" dataKey="Actual" stroke={isDarkMode ? "#ff7043" : "#82ca9d"} />
								</LineChart>
							</ResponsiveContainer>
						</div>
					</div>
					...
					<div
						className={`w-full md:w-[30%] h-full rounded-3xl py-6 px-4 ${isDarkMode
							? "bg-zinc-800 hover:bg-zinc-700 text-white fade-in"
							: "bg-zinc-100 hover:bg-zinc-200 text-zinc-900 fade-out"
							}`}
					>
						...
						<div className={` ${isDarkMode ? "text-white" : "text-black"} rounded-lg`}>
							{revenueDataCountry.map((data, index) => (
								<div key={index} className="mb-2">
									...
									<div
										className={`w-full h-2 ${isDarkMode ? "bg-zinc-700" : "bg-gray-200"} rounded-full`}
									>
										<div
				...
				>
					...
					<div
						className={`w-full md:w-[70%] rounded-3xl overflow-y-scroll py-6 px-4 ${isDarkMode
							? "bg-zinc-800 hover:bg-zinc-700 text-white fade-in"
							: "bg-zinc-100 hover:bg-zinc-200 text-black fade-out"
							}`}
						style={{ maxHeight: '380px' }} // Ensures scroll if table content overflows
					>
							...
							<table className="min-w-full table-auto text-left border-collapse">
								...
								<tbody>
									{tableData.map((item, index) => (
										<tr key={index} className="hover:bg-zinc-700"> {/* Hover effect */}
											{tableHeaders.map((header, index) => (
					...
					{/* Total Sales Section */}
					<div
						className={`w-full md:w-[30%] h-auto rounded-3xl py-6 px-4 ${isDarkMode
							? "bg-zinc-800 hover:bg-zinc-700 text-white fade-in"
							: "bg-zinc-100 hover:bg-zinc-200 text-black fade-out"
							}`}
					>

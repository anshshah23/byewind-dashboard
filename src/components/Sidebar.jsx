...
function ExpandableMenu({
	...
	return (
		<li className="cursor-pointer">
			<div
				className={`flex text-md py-2 pl-3 rounded-lg items-center transition-colors duration-300 ${isDarkMode
			...
			>
				...
				>
					{isOpen ? (
						<FaChevronUp
							className={`text-${isDarkMode ? "zinc-400 fade-in" : "fade-out zinc-500"
								}`}
							style={{ fontSize: "12px" }}
						/>
					) : (
				...
				<Icon
					className={`mx-2 text-${isDarkMode ? "zinc-300 fade-in" : "fade-out zinc-800"
						}`}
					style={{ fontSize: "18px" }}
				/>
				<span
					className={`text-${isDarkMode ? "zinc-300 fade-in" : "fade-out zinc-800"
						} text-md font-normal`}
				>
					{label}
				</span>
				...
				{isOpen && (
					...
					>
						{menuItems.map((item, index) => (
							<motion.li
								key={index}
								className={`py-2 pl-4 cursor-pointer rounded-lg transition-colors duration-300 ${isDarkMode ? "hover:bg-zinc-600 fade-in" : "hover:bg-zinc-300"
									} ${activeTab === item
										? isDarkMode
											? "active-tab-dark"
											: "active-tab"
...
export default function Sidebar() {
	...
	return (
		<motion.div
			...
			className={`h-screen font-sans ${isDarkMode ? "bg-zinc-900 text-zinc-300" : "bg-white text-zinc-700"} 
                md:w-52 top-0 left-0 z-50 shadow-lg transition-all duration-300`}
		>
			<div
				className={`w-52 ${isLeftClose ? "hidden" : "block"} transition-bg ${isDarkMode
			...
			>
					...
					<div className="font-normal text-md">
						<div className="mb-4 ml-2">
							<div className="flex mb-2">
								<button
									className={`mr-4 ${activeTab1 === "Favorites"
										? isDarkMode
							...
							<ul>
								{(activeTab1 === "Favorites" ? favorites : recent).map(
									(item) => (
										...
										>
											<span
												className={`rounded-full h-2 w-2 ${selectedItem1 === item
													? isDarkMode
														? "bg-zinc-300"
														: "bg-zinc-700"
													: "bg-zinc-300"
				...
				<div className="mb-4">
					<p
						className={`text-${isDarkMode ? "zinc-400" : "zinc-500"
					...
					<ul>
						{[
							...
							<li
								key={label}
								className={`flex items-center text-md py-2 pl-4 cursor-pointer transition-colors duration-300 ease-in-out ${activeTab === label
									? isDarkMode
										? "active-tab-dark"
										: "active-tab"
							...
							>
								...
								/>
								<span
									className={`text-${isDarkMode ? "zinc-300" : "zinc-800"
										} text-md font-normal`}
								>
									{label}
								</span>

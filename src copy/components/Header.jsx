	...
	return (
		...
		>
			...
			<div className="flex items-center gap-x-5">
				...
				{isDarkMode ? (
					<GoSun onClick={toggleDarkMode} className="cursor-pointer hover:text-yellow-300 duration-300" />
				) : (
					<GoSun onClick={toggleDarkMode} className="cursor-pointer hover:text-yellow-300 duration-300" />
				)}
				<FaClockRotateLeft
					onClick={handleClockClick}
					className={`cursor-pointer transition-transform hover:text-yellow-300 duration-300 ${rotate ? "rotate-animation" : ""
						}`}
					style={{ fontSize: "16px" }}
				/>
				<FaRegBell
					onClick={handleBellClick}
					className={`cursor-pointer hover:text-yellow-300 duration-300 transition-transform ${animate ? "animate-bell" : ""
						}`}
					style={{ fontSize: "16px" }}
				/>
				<div>
